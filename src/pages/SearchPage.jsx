...
import { verifyCity } from '@/services/weatherService';
...
function SearchPage() {
  ...
  const { data: apiSuggestions = [] } = useCitySuggestions(trimmed);
  const suggestions = trimmed.length > 2 ? apiSuggestions : [];
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <form onSubmit={onSubmit} className="flex gap-3 relative">
        ...
        {suggestions.length > 0 && (
          <ul className="absolute top-full border rounded bg-white mt-1">
            {suggestions.map((s) => (
              <li
                key={s.id}
